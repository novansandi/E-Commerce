...
class globalVars with ChangeNotifier {
  ...
  Future<bool> checkExist(docID) async {
    try {
      var doc = await UsersInformation.doc(docID).get();
      return doc.exists;
    } catch (e) {
      return false;
    }
  ...
  Future getUserCart(User u) async {
    List cart = [];
    bool exist = await checkExist(u.uid);
    if (!exist) {
      UsersInformation.doc(u.uid).set({"cart": cart});
    }
